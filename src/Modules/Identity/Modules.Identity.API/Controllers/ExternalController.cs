...
internal partial class AccountController
{
    ...
    {
        Result<List<LoginProvider>> result = await queryDispatcher.QueryAsync(new GetExternalProvidersQuery());
    ...
    public async Task<IActionResult> GetCallbackAsync([FromQuery] string provider, [FromQuery] string returnUrl)
    {
        ...
        Result<AuthenticationProperties> result = await commandDispatcher.SendAsync(command);
    ...
    public async Task<IActionResult> GetExternalLoginsAsync([FromRoute] Guid userId)
    {
        ...
        Result<List<ExternalLoginInfo>> result = await queryDispatcher.QueryAsync(query);
